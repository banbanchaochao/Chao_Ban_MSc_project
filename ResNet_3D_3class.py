...
label_5_3D_file = np.load('corticalThicknessMatrix_label_5.npz')
label_5_data = label_5_3D_file['Matrix']

label_for_train_and_test = np.zeros((9, 3))

label_for_train_and_test[0:3, 0] = 1

label_for_train_and_test[3:6, 1] = 1

label_for_train_and_test[6:9, 2] = 1
...
def get_Convolutional_neural_network(x):
    ...
    W_linear = tf.Variable(tf.random_normal([1024, 3], stddev=0.1))

    b_fc = tf.Variable(tf.constant(0.1, shape=[1024]))
    b_linear = tf.Variable(tf.constant(0.1, shape=[3]))
...
def save_model(session, ith_net):
    file_name = './3D_model_3class_' + str(int(ith_net)) + '/'
...
def main():
    ...
    for ith_net in range(1, 101):
        ...
        label_5_id_test_new = random.sample(range(1082), 72)
        ...
        with tf.Session() as sess:
            ...
            for indexIter in range(iter_num):
                ...
                if (indexIter+1) % train_batch == 0:
                    ...
                    for test_indexIter in range(23):
                        ...
                        label_5_id_test = label_5_id_test_new[lower_index_test:upper_index_test]

                        for k in range(3):
                            testX[k] = label_1_data[label_1_id_test[k]]
                            testX[k + 3] = label_3_data[label_3_id_test[k]]
                            testX[k + 6] = label_5_data[label_5_id_test[k]]
            ...
            document_name = '3D_3class_curve_' + str(int(ith_net)) + '.npz'
